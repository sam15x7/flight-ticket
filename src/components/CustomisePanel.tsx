...
export function CustomisePanel() {
  ...
  useEffect(() => {
    ...
    if (!ticketData && selectedFlight) {
      ...
      const initialData = {
        platform: { name: 'MyTrip', tagline: 'Travel Partner', logoBase64: 'https://cdn.brandfetch.io/idjNS9diAZ/theme/dark/logo.svg?c=1bxid64Mup7aczewSAYMX&t=1772905499480' },
        passenger: {
          name: 'John Doe',
          dob: '01 Jan, 1990',
    ...
    try {
      ...
      for (const leg of pdfData.itinerary.legs) {
        if (leg.airline.iataCode) {
           const effectiveUrl = getAirlineLogoUrl(leg.airline.iataCode);
           if (effectiveUrl) {
             try {
